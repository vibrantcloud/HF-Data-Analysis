...
elif total_performance < variable_hours and variable_hours > minx:
     print(variable_hours)
elif minx > total_performance and minx > variable_hours:
    print(minx)
...
conditions = [(df['Total Sales'] > df['Total Forecast']) & (df['Total Perf'] < 0),
              (df['Total Perf'] >= 0),
              (df['Total Perf'] < df['Variable Hours']) & (df['Variable Hours'] > df['Min']),
              (df['Min'] > df['Total Perf']) & (df['Min'] > df['Variable Hours']),
              (df['Total Perf'] > df['Min']) & (df['Total Perf'] > df['Variable Hours'])
             ]
outputs = [0,df['Total Perf'], df['Variable Hours'],df['Min'],df['Total Performance']]
